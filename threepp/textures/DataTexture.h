...
protected:
  ...
  DataTexture(const TextureOptions &options, const TextureData::Ptr data, size_t width, size_t height, bool compressed=false)
     : Texture(options, texture::Typer(this), false, 1), _data(data), _width(width), _height(height), _compressed(compressed)
  {
    needsUpdate();
  }
...
protected:
  DataCubeTexture(const TextureOptions &options, const std::array<TextureData::Ptr, CubeTexture::num_faces> &datas,
     ...
     : CubeTexture(options, texture::Typer(this), false, 1),_datas(datas), _width(width), _height(height), _compressed(compressed)
  {
  }
