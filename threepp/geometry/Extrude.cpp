...
struct Builder : private ExtrudeOptions
{
  ...
  vector<extras::shapeutils::Face> faces;
  ...
  void addShape(extras::Shape::Ptr shape);
...
void Builder::addShape(extras::Shape::Ptr shape)
{
  setShapeGroup();
  ...
  bool reverse = !extras::shapeutils::isClockWise(vertices);
  if (reverse) {
    ...
    for (auto &hole : holes) {
      ...
      if (extras::shapeutils::isClockWise(hole)) {
  ...
  faces = extras::shapeutils::triangulateShape(vertices, holes);
...
Extrude::Extrude(const std::vector<extras::Shape::Ptr> &shapes, const ExtrudeOptions &options)
{
  ...
  for (extras::Shape::Ptr shape : shapes) {
